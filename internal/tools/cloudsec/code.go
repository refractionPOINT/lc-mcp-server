...
// codeFindingParams is the findings selector as the code lane uses it.
//
// `repo` is NOT redeclared here. The shared selector already declares it, validates it
// and case-folds it, and a second declaration of the same name would be one more place
// for the two descriptions to drift apart. What the code lane adds is a REQUIREMENT
// rather than a different parameter, and the tool description carries it.
func codeFindingParams(paging bool) []mcp.ToolOption {
	return findingSelectorParams(paging)
}
...
func registerCodeFindings() {
	register(toolDef{
		...
		description: "List the AppSec code lane's findings for one or more repositories — dependency vulnerabilities (SCA), secrets, infrastructure-as-code " +
			...
			"pushed local scan, 'sarif-ingest'/'cyclonedx-ingest' for a converted document). To FILTER on it use 'source', which groups those tokens server-side: " +
			"'hosted' is the scan LimaCharlie ran, 'ingest' is a document your own pipeline pushed, 'other' is the source control's own detectors, and 'none' is a " +
			"finding with no code provenance at all. Filter with 'source' rather than folding 'detected_via' yourself — a page-at-a-time fold makes every count a " +
			"count of one page. " + codeLaneNote,
		readOnly: true,
...
func codeFindingsRequest(args map[string]interface{}) (string, lc.Dict, *mcp.CallToolResult) {
	...
	// `repo` is the SHARED selector's, not a second one: addFindingSelector validates
	// each key and case-folds it to the spelling the column actually stores. This tool
	// only NARROWS it, by requiring one outside facet mode. Parsing the raw argument
	// again here would reintroduce the two bugs that selector exists to prevent — an
	// unfolded key that is syntactically perfect and matches nothing, and a junk element
	// dropped so the read runs over fewer repositories than were asked for.
	q := lc.Dict{}
	if errResult := addFindingSelector(q, args, !facets); errResult != nil {
		return "", nil, errResult
	}
	repos, _ := q["repo"].([]string)
...
//
// Rather than assert a transformation — an earlier version of this suggested a
// lower-cased key, back when the fold did not exist and the guess was right only by
// luck — it asks /code/repos what the repository is actually called. That is one bounded
// extra request, spent only on an empty page under a repo filter, and it answers with a
// fact: either the real key, or that no such repository is in the inventory.
//
...
func emptyRepoPageNote(ctx context.Context, org *lc.Organization, q lc.Dict, resp map[string]interface{}) string {
	...
	return "no findings matched, and '" + asked + "' is not a repository this organization has. The 'repo' filter is matched EXACTLY against the stored key, " +
		"which is ASCII lower-cased in both segments (a finding's code.repo_name is the platform's display casing, which can differ in case but not otherwise). " +
		"This organization has: " + strings.Join(known, ", ")
}
