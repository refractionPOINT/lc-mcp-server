...
func TestCodeFindingsWillNotListUnscoped(t *testing.T) {
	...
	// ask over one repository with nothing in the response saying so; this tool cannot
	// even fall back to an unscoped read, since that is the whole-estate worklist it
	// exists to avoid returning.
	t.Run("a blank repo ends the call rather than narrowing it silently", func(t *testing.T) {
		_, _, errResult := codeFindingsRequest(map[string]interface{}{
...
// transformation. Casing itself is no longer the trap — the shared selector folds what a
// caller passes, to match a column that is ASCII lower-cased in both segments — but a key
// that is simply wrong, or a repository absent from the inventory, still returns an empty
// page and looks identical to one. Only /code/repos can tell those apart.
...
// The code lane declares no selector of its own beyond requiring a repository: it takes
// the shared one whole, so a parameter added there reaches these tools without being
// redeclared. Pinned because the alternative — a second copy of the same dimension — is
// how the two descriptions drift into contradicting each other.
func TestCodeFindingsInheritsTheSharedSelector(t *testing.T) {
	_, q, errResult := codeFindingsRequest(map[string]interface{}{
		"repo":   []interface{}{"acme/api"},
		"source": "ingest",
	})
	require.Nil(t, errResult)
	assert.Equal(t, "ingest", q["source"])

	// And its refusals are the shared ones, so a shape the wire cannot carry ends the
	// call here too rather than reading the whole estate under an active-looking filter.
	_, _, bad := codeFindingsRequest(map[string]interface{}{
		"repo":   []interface{}{"acme/api"},
		"source": map[string]interface{}{"eq": "hosted"},
	})
