...
// ExchangeAPIKeyForJWT exchanges a UID + API key for a LimaCharlie JWT
// This is used for AuthModeUIDKey when we need a specific OID scope.
// Pass oid="-" for user-level operations (caller identity only, no org permissions)
// This avoids the SDK generating an "all orgs" JWT which can be too large.
func ExchangeAPIKeyForJWT(uid, apiKey, oid string, logger *slog.Logger) (string, error) {
	if uid == "" {
	...
	// Prepare form data matching the SDK's RefreshJWT behavior
	formData := url.Values{}
	formData.Set("uid", uid)
	formData.Set("secret", apiKey)
	...
	logger.Debug("Exchanging API key for LimaCharlie JWT",
		"uid", uid,
		"oid", oid)
	...
	logger.Info("Successfully exchanged API key for LimaCharlie JWT",
		"uid", uid,
		"oid", oid,
		"jwt_prefix", safePrefix(jwtResp.JWT, 20))
